...
//  RandomRun
//
//  Created by Fabian Canas on 11/14/14.
//  Copyright (c) 2014 MojoTech. All rights reserved.
//

import MapKit

class RunRenderer: MKOverlayRenderer {
    var color: NSColor = NSColor.blueColor()
    var chevronColor: NSColor = NSColor.cyanColor()
    ...
    func pointDist(point1: CGPoint!, point2: CGPoint!) -> CGFloat {
        return sqrt(pow(point1.x - point2.x, 2.0) + pow(point1.y - point2.y, 2.0))
    }
    
    func chevronToPoint(path: CGMutablePathRef!, point: CGPoint, size: CGFloat!, offset: CGFloat) -> CGFloat {
